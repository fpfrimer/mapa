...
\begin{tikzpicture}[
  ...
  box/.style={draw=Line,rounded corners=2pt,fill=Paper,minimum width=34mm,minimum height=12mm,align=center},
  flow/.style={-{Latex[length=2mm]},thick,color=Slate}
]
\node[box] (browser) {Navegador\\HTML, CSS e JavaScript};
\node[box,right=of browser] (node) {Servidor Node.js\\HTTP + API};
\node[box,right=of node] (data) {Diretório de dados\\JSON protegido};
\draw[flow,<->] (browser) -- node[midway,above,font=\scriptsize] {HTTPS/HTTP interno} (node);
\draw[flow,<->] (node) -- node[midway,above,font=\scriptsize] {fila + troca atômica} (data);
\node[box,below=of node] (systemd) {systemd\\ambiente e reinício};
\draw[flow] (systemd) -- (node);
\end{tikzpicture}
\caption{Componentes e limites da implantação. O diretório de dados não é publicado como conteúdo estático.}
